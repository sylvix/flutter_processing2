...
mixin SketchMathRandom {
  ...
  /// Каждый раз, когда вызывается функция `random()`, она возвращает непредсказуемое значение в указанном диапазоне.
  /// Если функции передан только один параметр, она вернёт число с плавающей запятой в диапазоне от нуля до значения параметра [bound1].
  /// Например, `random(5)` возвращает значения от 0 до 5 (начиная с нуля и до, но не включая, 5).
  /// Если указаны два параметра, функция вернёт [double] в диапазоне между этими двумя значениями.
  /// Например, `random(-5, 10.2)` возвращает значения, начиная с -5 и до (но не включая) 10.2.
  ///
  /// Для преобразования случайного числа с плавающей запятой в целое число используйте метод [toInt()].
